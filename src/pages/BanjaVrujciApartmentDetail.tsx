...
const BanjaVrujciApartmentDetail = () => {
  ...
  useEffect(() => {
    const loadApartment = async () => {
      try {
        ...
        if (found) {
          document.title = `${found.name} - Banja Vrujci Apartments`;
  ...
  if (loading) {
    return (
      <>
        <Header />
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[hsl(var(--nature-muted))] via-white to-[hsl(var(--nature-muted))] pt-20">
          <div className="text-center">
            <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-[hsl(var(--nature-primary))] mx-auto mb-4"></div>
            <h1 className="text-2xl font-bold text-[hsl(var(--nature-accent))] mb-4 font-playfair">{t('apartment.loading')}</h1>
          </div>
        </div>
        <Footer />
      </>
  ...
  if (!apartment) {
    return (
      <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[hsl(var(--nature-muted))] via-white to-[hsl(var(--nature-muted))] pt-20">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-[hsl(var(--nature-accent))] mb-4 font-playfair">{t('apartment.notFound')}</h1>
            <Button onClick={() => navigate(`/${lang || 'en'}/banja-vrujci`)} className="bg-[hsl(var(--nature-primary))] text-white hover:bg-[hsl(var(--nature-accent))]">
  ...
  return (
    <>
      <Header />
      <main className="min-h-screen bg-gradient-to-br from-[hsl(var(--nature-muted))] via-white to-[hsl(var(--nature-muted))] pt-20">
        <div className="container-luxury py-8">
          <Button 
            onClick={() => navigate(`/${lang || 'en'}/banja-vrujci/apartments`)} 
            variant="ghost" 
            className="mb-6 text-[hsl(var(--nature-primary))] hover:text-[hsl(var(--nature-accent))] hover:bg-white/50"
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Nazad na apartmane
          </Button>

          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl p-8 relative before:content-[''] before:absolute before:inset-x-0 before:top-0 before:h-1 before:bg-gradient-to-r before:from-[hsl(var(--nature-primary))] before:to-[hsl(var(--nature-accent))]">
            <h1 className="text-3xl font-bold text-[hsl(var(--nature-accent))] font-playfair mb-6">
              {apartment.name}
            </h1>
                      ...
                      {apartment.images.map((_, index) => (
                        <button
                          ...
                          className={`w-3 h-3 rounded-full transition-colors ${
                            index === currentImageIndex ? 'bg-[hsl(var(--nature-primary))]' : 'bg-gray-300'
                          }`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              <div>
                <h3 className="text-xl font-semibold text-[hsl(var(--nature-accent))] mb-4 font-playfair">{t('modal.details')}</h3>
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <Users className="h-5 w-5 text-[hsl(var(--nature-primary))]" />
                    <span className="font-app text-[hsl(var(--nature-muted-foreground))]">{t('modal.guests', { count: apartment.max_guests })}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <Square className="h-5 w-5 text-[hsl(var(--nature-primary))]" />
                    <span className="font-app text-[hsl(var(--nature-muted-foreground))]">{t('modal.size', { size: apartment.size_m2 })}</span>
                  </div>
              ...
              <div>
                <h3 className="text-xl font-semibold text-[hsl(var(--nature-accent))] mb-4 font-playfair">Dostupnost</h3>
            ...
            <Button
              ...
              size="lg"
              className="w-full bg-[hsl(var(--nature-primary))] text-white hover:bg-[hsl(var(--nature-accent))] transition font-app font-semibold text-lg py-3"
            >
              {t('modal.reserve')}
            </Button>
          </div>
...
export default BanjaVrujciApartmentDetail;